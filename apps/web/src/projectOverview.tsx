...
function paperMetadataPercent(paper: PaperRecord): number {
  const values = [paper.title, paper.authors, paper.year, paper.publication_venue, paper.abstract, paper.identifiers ?? paper.doi, paper.keywords];
...
export function ProjectOverviewPage({
  ...
  const paperCounts = useMemo(() => ({ pdf: papers.filter((paper) => paper.pdf_access_status === "pdf_ready").length, extracted: papers.filter((paper) => paper.processing_state?.extraction_status === "completed").length, incomplete: papers.filter((paper) => paperMetadataPercent(paper) < 100).length }), [papers]);
