...
type DirtySurface = "project" | "profile" | "paper" | "note";
...
function AppShell({ children, page, onNavigate, connectionState, connectionMessage, onOpenOnboarding }: { children: ReactNode; page: PageId; onNavigate: (id: PageId) => void; connectionState: ConnectionState; connectionMessage: string; onOpenOnboarding: () => void }) {
  const activeNavigation = page === "profile" || page === "project" || page === "papers" || page === "notes" ? "projects" : page === "paper" ? "reading" : page;
  return (
...
export function App() {
  ...
  const [noteScopeType, setNoteScopeType] = useState<NoteRecord["scope_type"]>("project");
  const [activeNotePaper, setActiveNotePaper] = useState<PaperRecord | null>(null);
  ...
  function navigate(nextPage: PageId) {
    ...
    if (page === "notes" && nextPage !== "notes" && noteHasUnsavedChanges) {
      setPendingNavigation(nextPage);
      setPendingDirtySurface("note");
      return;
    }
  ...
  function discardProjectEditsAndNavigate() {
    ...
    else if (pendingDirtySurface === "note") setNoteHasUnsavedChanges(false);
  ...
  async function connectWorkspace(action: WorkspaceAction) {
    ...
    try {
      ...
      setNoteCreateRequested(false);
      setActiveNotePaper(null);
      setPendingNavigation(null);
      setPendingDirtySurface(null);
  ...
  function requestWorkspaceChange(operation: "create" | "open") {
    ...
    if (projectHasUnsavedChanges || profileHasUnsavedChanges || paperHasUnsavedChanges || noteHasUnsavedChanges) {
  ...
  function openNotes(scopeType: NoteRecord["scope_type"] = "project", paper: PaperRecord | null = null, create = false) {
    setNoteScopeType(scopeType);
    setActiveNotePaper(paper);
    setNoteCreateRequested(create);
    navigate("notes");
  }
  ...
  return (
    <AppShell page={page} onNavigate={navigate} connectionState={connectionState} connectionMessage={connectionMessage} onOpenOnboarding={() => setModal("onboarding")}>
      ...
      {page === "project" ? <ProjectOverviewPage project={activeProject} companionUrl={companionUrl} sessionToken={sessionToken} workspaceId={workspace?.workspace_id ?? null} workspaceName={workspace?.name ?? null} workspaceState={workspaceState} connectionState={connectionState} onNavigate={navigate} onOpenResearchProfile={openResearchProfile} onOpenPapers={openPapers} onOpenNotes={(create = false) => openNotes("project", null, create)} onProjectInvalid={handleProjectInvalid} /> : null}
      {page === "papers" ? <PapersPage key={workspaceContextVersion} project={activeProject} companionUrl={companionUrl} sessionToken={sessionToken} workspaceId={workspace?.workspace_id ?? null} workspaceState={workspaceState} connectionState={connectionState} createRequested={paperCreateRequested} onCreateRequestHandled={() => setPaperCreateRequested(false)} onNavigate={navigate} onDirtyChange={setPaperHasUnsavedChanges} onOpenNotes={(paper) => openNotes("paper", paper)} /> : null}
      {page === "notes" ? <NotesPage key={`${workspaceContextVersion}-${noteScopeType}-${activeNotePaper?.paper_id ?? "project"}`} project={activeProject} paper={activeNotePaper} scopeType={noteScopeType} companionUrl={companionUrl} sessionToken={sessionToken} workspaceId={workspace?.workspace_id ?? null} workspaceState={workspaceState} connectionState={connectionState} createRequested={noteCreateRequested} onCreateRequestHandled={() => setNoteCreateRequested(false)} onNavigate={navigate} onDirtyChange={setNoteHasUnsavedChanges} /> : null}
      ...
      {page === "settings" ? <SettingsPage category={settingsCategory} onCategoryChange={setSettingsCategory} companionUrl={companionUrl} sessionToken={sessionToken} workspaceId={workspace?.workspace_id ?? null} connectionState={connectionState} /> : null}
      <Modal open={modal === "onboarding"} eyebrow={pendingWorkspaceAction ? "Unsaved research edits" : "Onboarding"} title={pendingWorkspaceAction ? "Change workspace?" : "Set up your local-first workspace"} onClose={pendingWorkspaceAction ? cancelWorkspaceChange : () => setModal(null)}>
        {pendingWorkspaceAction ? <>
          <p className="modal-description">Changing workspace will replace the current project context. Keep editing, or discard the unsaved {projectHasUnsavedChanges ? "project" : paperHasUnsavedChanges ? "paper" : noteHasUnsavedChanges ? "note" : "research profile"} edits and continue with the requested {pendingWorkspaceAction.operation}.</p>
      ...
      <Modal open={pendingNavigation !== null} eyebrow={pendingDirtySurface === "profile" ? "Unsaved research profile" : pendingDirtySurface === "paper" ? "Unsaved paper" : pendingDirtySurface === "note" ? "Unsaved note" : "Unsaved project"} title={pendingDirtySurface === "profile" ? "Keep your unsaved profile edits?" : pendingDirtySurface === "paper" ? "Keep your unsaved paper edits?" : pendingDirtySurface === "note" ? "Keep your unsaved note edits?" : "Keep your unsaved edits?"} onClose={() => { setPendingNavigation(null); setPendingDirtySurface(null); }}>
        <p className="modal-description">This {pendingDirtySurface === "profile" ? "research profile" : pendingDirtySurface === "paper" ? "paper" : pendingDirtySurface === "note" ? "note" : "project"} has local edits that have not been saved. Choose whether to keep editing or leave without saving.</p>
...
function SettingsPage({ category, onCategoryChange, companionUrl, sessionToken, workspaceId, connectionState }: { category: SettingsCategory; onCategoryChange: (category: SettingsCategory) => void; companionUrl: string; sessionToken: string; workspaceId: string | null; connectionState: ConnectionState }) {
  return <div className="page"><PageHeader eyebrow="Settings" title="Workspace, AI, automation and privacy" /><div className="settings-layout"><Card className="settings-menu">{(Object.keys(settingsCopy) as SettingsCategory[]).map((key) => <Button key={key} variant={category === key ? "primary" : "secondary"} aria-pressed={category === key} onClick={() => onCategoryChange(key)}>{settingsCopy[key].label}</Button>)}</Card><Card className="settings-panel">{category === "ai" ? <><AiProviderSettings companionUrl={companionUrl} sessionToken={sessionToken} connectionState={connectionState} /><AiProcessingPanel companionUrl={companionUrl} sessionToken={sessionToken} workspaceId={workspaceId} connectionState={connectionState} /></> : <><p className="eyebrow">{settingsCopy[category].label}</p><div className="settings-rows">{settingsCopy[category].rows.map(([label, value]) => <ProfileRow label={label} key={label}>{value === "Healthy" ? <StatusPill>Healthy</StatusPill> : value}</ProfileRow>)}</div>{category === "privacy" ? <div className="callout">Paper content sent to an external AI provider always requires an outbound preview. Private notes are excluded by default.</div> : null}{category === "institution" ? <div className="callout">Institutional credentials, MFA codes and publisher session cookies are never stored.</div> : null}</>}</Card></div></div>;
}
