...
export function PaperSummarySection({
  ...
  return <>
    <section className="paper-summary-section" data-testid="paper-summary-section" aria-labelledby="paper-summary-heading">
      ...
      {active && !output ? <p className="summary-status" role="status">Latest request: <StatusPill tone={statusTone(active)}>{statusLabel(active)}</StatusPill>{active.error ? ` ${active.error.message}` : ""}</p> : null}
      ...
      {history.length ? <div className="paper-summary-history" data-testid="paper-summary-history" aria-label="Paper summary history"><div className="card-heading"><h4>Summary history</h4><span className="label">{history.length} event{history.length === 1 ? "" : "s"}</span></div>{history.slice(0, 5).map(({ record }) => <div className="paper-summary-history-row" key={record.processing_id}><span>{new Date(record.requested_at).toLocaleString()}</span><StatusPill tone={statusTone(record)}>{statusLabel(record)}</StatusPill><span className="muted-copy">{record.cache_disposition}</span></div>)}</div> : null}
